...
select bd.bidder_id,bd.bidder_name,(sum(case when b.bid_sttus='won' then 1 else 0 end)*100/cont(b.bid_status)) as winpercentage
from ipl_bidder_details bd join ipl_bidding_details b on bd.BIDDER_ID=b.BIDDER_ID
group by bd.BIDER_ID,bd.bidder_name
order by winperentage desc;

-- 2.	Display the number of matches conducted at each stadium with the stadium name and city.
select s.stadium_name,s.city,count(ms.match_id) as totalmatch from ipl_match_schedule ms join ipl_stadium s on ms.STADIUM_ID=s.STADIUM_ID
group by s.STADIUM_NAME,s.city;

-- 3.	In a given stadium, what is the percentage of wins by a team that has won the toss?
select s.stadium_id,s.stadium_name,(sum(case when m.match_winner=m.toss_winner then 1 else 0 end)*100/count(m.match_winner)) as win_percentage
from ipl_stadium s join ipl_match_schedule ms on s.STADIUM_ID=ms.STADIUM_ID
join ipl_match m on ms.MATCH_ID=m.MATCH_ID
group by s.STADIUM_ID,s.SADIUM_NAME;

-- 4.	Show the total bids along with the bid team and team name.
SELECT b.Bid_Team,t.Team_name,COUNT(b.Bid_Team) AS TotalBids
FROM IPL_Bidding_Details b
JOIN IPL_Team t ON b.Bid_Team=t.Team_Id
group by b.Bid_Team,t.Team_name;

-- 5.	Show the team ID who won the match as per the win details.
Select Match_Id,(case when match_winner=1 then team_id1 else team_id2 end) AS WinningTeamID,win_details
from IPL_Match;

-- 6.	Display the total matches played, total matches won and total matches lost by the team along with its team name.
select t.team_id,team_name,sum(matches_played)as totalmtch,sum(mathes_won)as won,sum(matches_lost) as lost
from ipl_team_standings ts join ipl_team t on ts.team_ID=t.TEAM_ID 
...
select bp.bidder_id,bd.bid_status,year(bd.bid_date) as year,sum(bp.total_points) as otal_bid_point
from ipl_bidding_details bd join ipl_bidder_points bp on bd.BIDDER_ID=bp.BIDDER_ID
JOIN ipl_match_schedule ms on bd.SCHEULE_ID
...
-- 13.	Write a query to display to calculate the total points month-wise for the 2017 bid year. sort the results based on total points in 
-- descending order and month-wise in ascending order.
...
-- Output columns should be: likeBidder Id, Ranks (optional), Total points, Highest_3_Bidders --> columns contains name of bidder, 
...
-- details of the students in the Student details table. Every time the students change their details like their mobile number, 
-- You need to update their details in the student details table.  Here is one thing you should ensure whenever the new students' details come, 
-- you should also store them in the Student backup table so that if you modify the details in the student details table, you will be having the 
-- old details safely.You need not insert the records separately into both tables rather Create a trigger in such a way that It should insert the
--  details into the Student back table when you insert the student details into the student table automatically.
